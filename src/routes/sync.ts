...
export async function syncRoutes(app: FastifyInstance): Promise<void> {
  /**
   * POST /api/sync
   * Manually trigger a data sync for AFL and World Cup.
    ...
    try {
      ...
      return reply.code(200).send({
        ...
        data: {
          ...
          message: `AFL: ${aflStats.synced} synced, ${aflStats.errors} errors | World Cup: ${worldCupStats.synced} synced, ${worldCupStats.errors} errors`,
        },
      });
    } catch (err) {
      app.log.error(err);
